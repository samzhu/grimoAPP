...
- [ ] Dependency upgrade path → official docs + registry + real resolver
      proof before accepting any version bump or API migration plan.
...
Treat every dependency upgrade as a two-layer change: the version
coordinate and the API surface. A registry page proves the artifact can
be downloaded; it does not prove the code still uses the right API.

For each upgrade request:

1. **Start from the concrete build file line.** Identify the exact
   manifest entry, lockfile entry, BOM import, version catalog entry,
   or package-manager constraint to change.
2. **Read official upgrade notes first.** Use official docs, release
   notes, migration guides, or official project docs to identify API
   renames, deprecated shims, new group IDs, property prefix changes,
   removed methods, and starter/module renames.
3. **Verify every coordinate in the canonical registry.** Check the
   exact group, artifact, and version. For Maven, prefer Sonatype
   Central / Maven Central pages or POM files; for npm, PyPI, crates,
   NuGet, etc., use their canonical registries.
4. **Inspect source/Javadoc/API when docs mention migration.** If
   official docs say `AgentApi` replaces `AgentModel`, validate the
   class/interface exists in the pinned artifact before writing the
   plan.
5. **Centralize family versions.** If several sibling artifacts must
   stay aligned, add one version property/BOM/import and make every
   sibling use it. Do not pin separate numbers unless the upstream
   explicitly releases them independently.
6. **Respect ecosystem-managed versions.** If the build's BOM already
   manages a transitive dependency, do not add an explicit lower version.
   Use the build resolver to confirm what actually wins.
7. **Treat groupId/package moves as a separate migration.** If docs say
   a project moved from `org.old` to `io.new`, do not silently cross the
   namespace during a patch upgrade. Record it as a distinct migration
   decision/spec unless the user explicitly asked for the namespace move.
8. **Compile before broad refactor.** Run the ecosystem compile/test
   command once after the version bump. Let compiler errors and
   deprecation warnings identify the real API delta; then update code to
   the new API path only.
9. **Prove the resolver output.** Run the ecosystem command that shows
   the selected dependency version:
   - Gradle: `./gradlew dependencyInsight --dependency <group:artifact> --configuration <classpath>`
   - Maven: `mvn dependency:tree -Dincludes=<groupId>:<artifactId>`
   - npm/pnpm/yarn: `<manager> why <package>` or lockfile inspection
   - Python: `pip show <package>`, `pipdeptree`, or lockfile inspection
   - Go: `go list -m all` / `go mod why`
   - Rust: `cargo tree -i <crate>`
10. **Document the proof.** In the architecture doc, record the exact
    version, primary import/module path, and evidence: official docs +
    registry + resolver command result. In user-facing summaries, state
    the concrete command and selected version.

Write the upgrade record in this project-agnostic shape:

```text
Upgrade: <dependency family or package>
Changed: <manifest/lockfile/version property path>
Official docs: <link + relevant migration note>
Registry proof: <canonical registry link showing exact coordinate/version>
API proof: <source/Javadoc/type docs showing new primary API and deprecated APIs avoided>
Code change: <old API/import/property> -> <new API/import/property>
Resolver proof: <command> selected <version>
Verification: <test/build command> passed, with no new deprecation warnings from this upgrade
Deferred: <separate namespace/groupId/runtime migration, if any>
```

**Upgrade acceptance rule:** the upgrade is not considered validated
until both are true:
- the build/test command passes without new deprecation warnings from
  the upgraded API; and
- resolver output proves the intended version is selected, especially
  when BOMs or transitive dependencies are involved.
