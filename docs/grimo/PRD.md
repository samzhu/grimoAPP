...
| D9 | Grimo 擁有 `Sandbox` 埠；**預設實作直接包裝 Testcontainers `GenericContainer`**（非 `DockerSandbox`），在 `start()` 前呼叫 `withFileSystemBind(worktree, "/work", RW)` | 原始碼 spike（2026-04-16）確認：`org.springaicommunity.sandbox.docker.DockerSandbox` **0.9.1** 在建構子中啟動容器，因此沒有 API 鉤點（`withBindMount`、`volume(...)`、customizer）在啟動前注入 bind-mount。子類化也失敗。直接使用 Testcontainers 可行；`DockerSandbox` 仍可用於*短暫檔案複製*情境，透過 `sandbox.files()` 實現。 | 向上游提 PR（長期仍需要，但不是 v1 阻礙）；原始 Docker Java API（更多程式碼，無 Testcontainers 輔助）；Podman（使用者更少）。 |
