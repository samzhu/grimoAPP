...
| Session | 對話會話 | Grimo 的事件源對話狀態，由 Grimo 擁有（而非底層 CLI）。以 `RecordingAgentSession` decorator 攔截 `AgentSession.prompt()`，持久化至 H2 event store（S017）。可在 CLI 切換後存活。 |
| Session Event | 對話事件 | `grimo_session_event` 表中的一筆 append-only 記錄。每輪對話產生 USER + ASSISTANT 兩筆 events，以 `payload_json` + `metadata_json` 儲存。`synthetic = true` 表示壓縮產生的合成事件（S014）。 |
| Session Projection | 對話投影 | `grimo_session` 表中的物化摘要。每個 session 一筆，含 `turn_count`、`total_tokens_in/out`、`total_duration_ms`。由 `TurnRecorder` 增量更新。 |
| Turn | 對話輪次 | 一次使用者輸入 + 一次 AI 回應。每輪產生 2 筆 session events（USER + ASSISTANT），共享同一 `turn_number`。 |
| ProviderMetadataExtractor | Provider 元資料萃取器 | Strategy pattern SPI，動態辨識 CLI provider 並萃取 provider-specific token 數據。新增 provider 只需一個 class，零 if-else。 |
| Compaction | 壓縮 | Session event store 的歷史壓縮機制。由 S014 定義 SPI（`CompactionTrigger` + `CompactionStrategy`）。壓縮後產生 `SUMMARY` synthetic event，原始 events 保留不刪除。 |
...
| Project | 專案 | Grimo 管理的工作單位，對應一個本地資料夾（可能有 git 也可能沒有）。每個 project 有自己的 chat session、task、memory。存於 `grimo_project` 表。 |
| Task | 任務 | 通用工作項目（研究、開發、文件、分析皆可），類似 GitHub Issue。可屬於某個 Project（`projectId` 非 null）或 Grimo 本身（`projectId` = null）。Lifecycle: OPEN → IN_PROGRESS → IN_REVIEW → DONE \| CANCELLED。來源可以是 Chat 對話、手動 REST API、或 Webhook。以 `labels` 區分任務類型（`bug`、`research`、`refactor`…）。開發類 Task 的 worktree/Docker/PR 執行管線由 S020 負責。存於 `grimo_task` 表。 |
| Session Type | 對話類型 | `GRIMO`（Grimo 本身的對話，不綁定 project）或 `PROJECT`（綁定特定 project 的對話）。決定 chat session 的 scope。 |
| Channel Binding | 頻道綁定 | 外部平台的頻道（Discord channel、Slack channel）與 Grimo Project 的對射關係。六邊形架構的入站適配器透過 binding 決定 session 的 projectId。 |
...
| SessionMemoryAdvisor | 會話記憶顧問 | 來自 Spring AI Session API 的事件源 session advisor。**不適用於 Grimo** — 掛載 `ChatClient` advisor chain，無法掛載到 `AgentSession.prompt()`（S011 POC 驗證）。Grimo 改用 `RecordingAgentSession` decorator（S017）。 |
