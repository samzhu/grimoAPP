...
**目標。** 提供 HTTP endpoint 取代 stdin/stdout REPL，讓 `chat` 和 `chat --resume` 可透過 `curl` / Postman / `MockMvc` 測試。降低端對端驗證成本 — 不再需要 pipe stdin 再 kill process。
**完成條件。** S018 ✅。

| # | 規格 | 點數 | 狀態 |
| --- | --- | --- | --- |
| S018 | Chat REST API（HTTP 對話端點） | 待估 | 🔲 |

### S018 — Chat REST API · 待估

**描述。** 新增**測試用** REST controller（`POST /api/test/chat`），接收 JSON message，回傳 AI 回應。目的是取代 `java -jar ... chat` + stdin pipe 的笨拙 E2E 測試方式，讓 `curl` 或 `MockMvc` 即可驗證 session 記錄、provider 偵測、token 萃取等功能。非正式產品 API — 僅供開發期間端對端驗證使用。

**依賴。** S007 ✅（主代理）、S017 ✅（session 記錄）。

**SBE（草稿）。**
- **AC-1** `POST /api/chat` 傳入 `{ "message": "hello" }` 回傳 AI 回應 JSON。
- **AC-2** 多次呼叫同一 sessionId 保持上下文。
- **AC-3** `POST /api/chat/resume` 恢復先前 session。
- **AC-4** E2E 測試可用 `MockMvc` 或 `WebTestClient` 執行，不需要 stdin pipe。

**估算。** 待 `/planning-spec S018` 時評估。
...
| S016 | `SkillCommandRunner` 使用 `System.out`（9 處）+ `System.err`（1 處），違反 development-standards §3「絕不使用 System.out」。`ChatCommandRunner`（S007/S011）也有同模式。需釐清 §3 是否豁免 CLI adapter 層 | drift | 低 | ✅ development-standards §3 已新增 CLI adapter 豁免條款 |
| S017 | `JdbcSessionEventAdapter.findBySessionId(sessionId, EventFilter)` 忽略 filter — 回傳全部事件。完整 filter 支援延後至 S014 或查詢功能需要時 | skip | 低 | 🔲 |
| S017 | `EventFilter.lastTurns(n)` 使用 `n * 2` 作為 lastN — 假設每 turn 只有 USER+ASSISTANT 兩筆 events。若未來加入 TOOL_CALL events，此公式需修正 | drift | 低 | 🔲 |
