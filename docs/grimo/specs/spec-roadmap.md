...
| S016 | `SkillCommandRunner` 使用 `System.out`（9 處）+ `System.err`（1 處），違反 development-standards §3「絕不使用 System.out」。`ChatCommandRunner`（S007/S011）也有同模式。需釐清 §3 是否豁免 CLI adapter 層 | drift | 低 | 🔲 |
