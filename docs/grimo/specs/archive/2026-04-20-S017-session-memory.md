...
> Spec: S017 | Size: S (11) | Status: ✅ Done
...
**POC: not required** — Event sourcing 為標準 pattern。JdbcTemplate + H2 + Jackson 已在專案中驗證（S011 POC）。Phase 0 pre-flight 未發現未驗證假設。

### Task Overview

| Task | Topic | ACs | Size |
|------|-------|-----|------|
| T1 | Schema + Domain + Module Boundary | AC-5, AC-6 | Infrastructure — module skeleton, schema, domain records, port interfaces |
| T2 | Event Recording + Projection + Provider SPI | AC-1, AC-2, AC-3, AC-7 | Core logic — JDBC adapters, TurnRecorder, ClaudeMetadataExtractor |
| T3 | Decorator + Registry Integration | AC-4 | Integration — RecordingAgentSession, RecordingAgentSessionRegistry(@Primary), SessionConfig |

### Execution Order

```
T1 (schema + domain + ports) → T2 (recording + projection) → T3 (decorator + registry)
```
...
| AC | Task | Description |
|----|------|-------------|
| AC-1 | T2 | Event Store per-turn events |
| AC-2 | T2 | Multi-turn append-only |
| AC-3 | T2 | Projection auto-materialization |
| AC-4 | T3 | Registry decorator transparent wrapping |
| AC-5 | T1 | Fork schema reservation |
| AC-6 | T1 | Modulith boundary compliance |
| AC-7 | T2 | Provider-agnostic metadata extraction |

### Task Files (Round 1 — completed, deleted)

- ~~`docs/grimo/tasks/2026-04-21-S017-T1.md`~~ PASS
- ~~`docs/grimo/tasks/2026-04-21-S017-T2.md`~~ PASS
- ~~`docs/grimo/tasks/2026-04-21-S017-T3.md`~~ PASS

### E2E Bug Fix POC Findings (2026-04-21)

POC: `AgentResponseMetadataPocIT` — 真實 Claude CLI 的 `AgentResponseMetadata` 實測結構：

```
AgentResponseMetadata (extends HashMap):
  key='inputTokens'   value=3        (Integer)    ← token key 正確
  key='outputTokens'  value=5        (Integer)    ← token key 正確
  key='phaseCapture'  value=PhaseCapture[...]      ← Claude-specific！
  getModel() = ""                                  ← 空字串，agent-client 0.12.2 未填入

AgentGenerationMetadata:
  getFinishReason() = "SUCCESS"
  providerFields = {} (空)
```

**結論：**
- `ClaudeMetadataExtractor.supports()` 不能靠 `model.startsWith("claude-")` — model 為空
- 改為檢查 `meta.containsKey("phaseCapture")` — Claude CLI 專有 key
- Token key 名稱 `inputTokens` / `outputTokens` 正確，只是因為 supports() 失敗所以沒被呼叫

### Task Files (Round 2 — E2E bug fixes)

- `docs/grimo/tasks/2026-04-21-S017-T4.md` — Fix provider detection + token extraction
- `docs/grimo/tasks/2026-04-21-S017-T5.md` — Fix --resume bypass of decorator
...
| AC | Status | Test |
|----|--------|------|
| AC-1: Event Store per-turn events | ✅ | `TurnRecorderTest.ac1_singleTurnProducesUserAndAssistantEvents` |
| AC-2: Multi-turn append-only | ✅ | `TurnRecorderTest.ac2_multiTurnAppendOnly` |
| AC-3: Projection auto-materialized | ✅ | `TurnRecorderTest.ac3_projectionAutoMaterialized` |
| AC-4: Registry decorator transparent | ✅ | `RecordingRegistryTest.ac4_createReturnsRecordingWrapper`, `ac4_findReturnsRecordingWrapper`, `ac4_findUnknownReturnsEmpty` |
| AC-5: Fork schema reserved | ✅ | `SchemaTest.ac5_parentIdColumnExists`, `ac5_forkTurnColumnExists`, `ac5_parentIdForeignKey` |
| AC-6: Modulith boundary | ✅ | `ModuleArchitectureTest.modulesVerify` |
| AC-7: Provider-agnostic metadata | ✅ | `TurnRecorderTest.ac7_providerAgnosticMetadata` |

### Key Findings

1. **H2 PostgreSQL mode + `AUTO_INCREMENT`** — `AUTO_INCREMENT` is invalid in H2's PostgreSQL compatibility mode. Must use `GENERATED BY DEFAULT AS IDENTITY` instead. This affected the original `grimo_conversation_turn` schema as well (never tested in PostgreSQL mode before). [Implementation note: §2.7 schema updated to use `GENERATED BY DEFAULT AS IDENTITY`.]

2. **H2 PostgreSQL mode type names** — `VARCHAR(36)` is reported as `CHARACTER VARYING` in `DatabaseMetaData.getColumns()`. Tests must handle both representations.

3. **`spring-modulith-events-jdbc` requires `events-jackson`** — The `JdbcEventPublicationAutoConfiguration` needs an `EventSerializer` bean, provided by `spring-modulith-events-jackson`. Added to `build.gradle.kts`. The events-jdbc module also needs its own `EVENT_PUBLICATION` schema, enabled via `spring.modulith.events.jdbc.schema-initialization.enabled=true`.

4. **`ObjectMapper` not auto-configured without web starter** — The project doesn't include `spring-boot-starter-web` (CLI-first, D11). Added `@ConditionalOnMissingBean ObjectMapper` in `SessionConfig` to provide Jackson for TurnRecorder serialization. When web starter is added later, this bean will be superseded by Spring Boot's auto-configured one.

5. **`AgentResponse.builder()` API** — Builder methods are `metadata()` and `results()`, not `withMetadata()` / `withGenerations()`. `AgentResponseMetadata` extends `HashMap<String,Object>` — providerFields are directly in the map.

6. **`MERGE INTO ... KEY (id)`** — H2 upsert syntax used for session projection. Compatible with PostgreSQL mode.

7. **`RecordingAgentSession.resume()` wraps returned session** — Not explicitly in spec §4 pseudocode but necessary for correctness: a resumed session must also be recorded.

### Correct Usage Patterns

```java
// Build stub AgentResponse for testing
var providerFields = Map.<String, Object>of("inputTokens", 100L, "outputTokens", 50L);
var metadata = new AgentResponseMetadata("claude-sonnet-4-20250514",
    Duration.ofMillis(2000), SESSION_ID, providerFields);
var genMeta = new AgentGenerationMetadata("SUCCESS", Map.of());
var generation = new AgentGeneration("response text", genMeta);
var response = AgentResponse.builder()
    .metadata(metadata)
    .results(List.of(generation))
    .build();

// Access token counts from AgentResponseMetadata (extends HashMap)
meta.get("inputTokens")  // returns Number (Long or Integer)
meta.getModel()           // "claude-sonnet-4-20250514"
meta.getDuration()        // Duration

// Access finish reason from AgentGenerationMetadata
response.getResult().getMetadata().getFinishReason()  // "SUCCESS"
```

### Pending Verification

None — all unit tests execute in `./gradlew test` with embedded H2.

### E2E Verification — Round 1 (2026-04-21, pre bug-fix)

初次 E2E 發現 3 個 bug（詳見下方 E2E Bug 清單）。已通過 T4+T5 修正。

### E2E Verification — Round 2 (2026-04-21, post bug-fix)

端對端以 `java -jar grimo-0.0.1-SNAPSHOT.jar` 驗證 `chat` 和 `chat --resume`。

| 測試項目 | 結果 | 說明 |
|---------|------|------|
| `chat` → H2 event store 寫入 | ✅ | 2 筆 events，provider="claude"，tokensIn=3，tokensOut=8 |
| `chat` → H2 projection 寫入 | ✅ | turn_count=1，total_tokens_in=3，total_tokens_out=8 |
| `chat --resume` → 上下文恢復 | ✅ | Claude 記住 PINEAPPLE（同 sessionId） |
| `chat --resume` → 事件記錄 | ✅ | 4 events total（Bug 1 修正驗證） |
| `chat --resume` → projection 累加 | ✅ | turn_count=2，tokens_in=6，tokens_out=17 |
| Provider 偵測 | ✅ | provider="claude"（Bug 2 修正：phaseCapture key 偵測） |
| Token 萃取 | ✅ | tokensIn/Out > 0（Bug 3 修正：連鎖效果消除） |

```
H2 evidence:
grimo_session_event (4 rows):
  seq=1  turn=1 USER      "hello, reply with just one word: PINEAPPLE"
  seq=2  turn=1 ASSISTANT "PINEAPPLE"        provider=claude tokensIn=3 tokensOut=8
  seq=33 turn=1 USER      "what was the word I asked you to say?"
  seq=34 turn=1 ASSISTANT "\n\nPINEAPPLE"    provider=claude tokensIn=3 tokensOut=9

grimo_session (1 row):
  id=99d7b2ca  provider=claude  turn_count=2  tokens_in=6  tokens_out=17  duration=14930ms
```
| `chat --resume` → 事件記錄 | ❌ **BUG** | resume 路徑繞過 `RecordingAgentSessionRegistry` |
| Provider 偵測 | ❌ **BUG** | 真實 CLI 的 `getModel()` 回傳空字串 |
| Token 萃取 | ❌ **BUG** | 因 provider 偵測失敗，tokensIn/Out = 0 |

**E2E Bug 清單（需設計修正）：**

**E2E-BUG-1：`--resume` 繞過 RecordingAgentSessionRegistry decorator。**
`MainAgentChatService.resumeChat()` 呼叫 `ClaudeSessionConnector.continueLastSession()` 直接建立 `AgentSession`，不經過 `AgentSessionRegistry.create()` / `find()`。S017 的 decorator 只包裝 registry 路徑，resume 路徑完全未被包裝。根因：S011 的 `ClaudeSessionConnector` 是 same-package helper，繞過 registry API。

**E2E-BUG-2：真實 Claude CLI 的 `AgentResponseMetadata.getModel()` 回傳空字串。**
`ClaudeMetadataExtractor.supports()` 用 `model.startsWith("claude-")` 偵測，但真實 CLI 回應的 model 為 `""`（非 `"claude-sonnet-4-20250514"`）。需要替代偵測策略（例如檢查 session registry 類型、或檢查 providerFields 特有 key）。

**E2E-BUG-3：Token 萃取因 provider 偵測失敗而全為 0。**
`ClaudeMetadataExtractor.extractTokens()` 未被呼叫，因為 `supports()` 回傳 false（Bug 2 的連鎖效果）。即使 provider 偵測修正，仍需驗證 `providerFields` 中的 `inputTokens` / `outputTokens` key 是否存在於真實 CLI 回應。

**E2E 修正期間發現的 infra 問題（已修正）：**

1. `spring.sql.init.mode` 從 `embedded` 改為 `always` — Boot 4.0.5 的 `embedded` 模式未觸發 H2 file mode 的 schema 初始化。
2. `TurnRecorded` record 移除 `AgentResponse` 欄位 — `spring-modulith-events-jdbc` 嘗試序列化事件至 `EVENT_PUBLICATION` 表，但 `AgentResponse` 無預設建構子導致 Jackson 失敗。改為在 `RecordingAgentSession` 中萃取可序列化欄位後才發佈事件。
3. `@ApplicationModuleListener` 改為 `@EventListener` — 配合上述序列化修正，避免 Modulith event persistence 的 Jackson 問題。

### Design Drift

- §2.7 schema: `AUTO_INCREMENT` → `GENERATED BY DEFAULT AS IDENTITY` (H2 PostgreSQL mode compatibility). [Implementation note added.]
- §4 `TurnRecorder`: spec pseudocode omits `branch` parameter in `SessionEvent` constructor — implementation passes `null` (correct for root session).
- §4 `RecordingAgentSession.resume()`: spec says "delegate" only — implementation also wraps returned session in RecordingAgentSession for recording continuity.
- §5 File Plan: `SessionConfig.java` listed as T3 target but created in T2 (ObjectMapper bean needed by TurnRecorder).

### Tech Debt

| Source | Description | Type | Priority |
|--------|-------------|------|----------|
| S017 | `JdbcSessionEventAdapter.findBySessionId(sessionId, EventFilter)` ignores filter — returns all events. Full filter support deferred to when S014 or query features need it. | skip | 低 |
| S017 | `EventFilter.lastTurns(n)` uses `n * 2` for lastN — assumes USER+ASSISTANT per turn. If TOOL_CALL events are added later, this formula breaks. | drift | 低 |

---

## QA Review — 2026-04-21

**審查者：** 獨立 QA 代理（verifying-quality 技能）
**對象：** S017 v3.1（Provider-Agnostic Event-Sourced Session Memory）
**審查依據：** `qa-strategy.md`、`development-standards.md`
...
| `./gradlew test --rerun-tasks` | BUILD SUCCESSFUL（全部 11 個 S017 相關測試通過） |

**測試執行證據：**

```
TEST-io.github.samzhu.grimo.session.TurnRecorderTest.xml
  tests="4" skipped="0" failures="0" errors="0"
  - [S017] AC-1: single turn produces USER + ASSISTANT events          PASS
  - [S017] AC-2: 3 turns produce 6 events with ascending sequence      PASS
  - [S017] AC-3: session projection auto-materialized after 3 turns    PASS
  - [S017] AC-7: provider-agnostic metadata via ClaudeMetadataExtractor PASS

TEST-io.github.samzhu.grimo.session.RecordingRegistryTest.xml
  tests="3" skipped="0" failures="0" errors="0"
  - [S017] AC-4: create() returns RecordingAgentSession...             PASS
  - [S017] AC-4: find() returns RecordingAgentSession wrapping...      PASS
  - [S017] AC-4: find() for unknown sessionId returns empty            PASS

TEST-io.github.samzhu.grimo.session.SchemaTest.xml
  tests="3" skipped="0" failures="0" errors="0"
  - [S017] AC-5: grimo_session has parent_id VARCHAR(36) nullable       PASS
  - [S017] AC-5: grimo_session has fork_turn INT nullable column        PASS
  - [S017] AC-5: parent_id has FOREIGN KEY referencing grimo_session    PASS

TEST-io.github.samzhu.grimo.ModuleArchitectureTest.xml
  tests="1" skipped="0" failures="0" errors="0"
  - AC-1 ApplicationModules.verify() passes on the live module graph    PASS
```

---

### Layer 2 — Coverage & Integration

**覆蓋率：** JaCoCo 報告未在本次執行中產生（`./gradlew test` 未觸發 `jacocoTestReport` task）。`application/service/` 和 `domain/` 為 QA 策略目標（≥75% 行覆蓋）。TurnRecorder 和 SessionHistoryService 有完整的正向路徑測試，但以下路徑未被覆蓋：

- `TurnRecorder.on()` 中 `extractors` 為空串列（`resolveProvider` 回傳 "unknown" 路徑）
- `TurnRecorder.toJson()` 的例外路徑（`JsonProcessingException` 分支）
- `SessionHistoryService` 三個方法雖有實作但無對應測試

**整合測試：** 無 T4 IT 類別（預期 — spec size S，qa-strategy §5 規定 S size 只需 T0+T1+T2+Modulith verify）。

---

### Layer 3 — Manual Verification Readiness

本 spec 無需人工互動驗證（所有 AC 可自動化）。N/A。

---

### Layer 4 — Testability Gate: AC 分類

| AC | 分類 | 對應測試 | 結論 |
|----|------|---------|------|
| AC-1: USER + ASSISTANT 兩筆 events | VERIFIED | `TurnRecorderTest.ac1_singleTurnProducesUserAndAssistantEvents` | PASS |
| AC-2: 多輪 append-only + turn 遞增 | VERIFIED | `TurnRecorderTest.ac2_multiTurnAppendOnly` | PASS |
| AC-3: Projection 自動物化 | VERIFIED | `TurnRecorderTest.ac3_projectionAutoMaterialized` | PASS |
| AC-4: Registry decorator 透明包裝 | VERIFIED | `RecordingRegistryTest` 三個測試 | PASS |
| AC-5: Fork schema 預留 | VERIFIED | `SchemaTest` 三個測試 | PASS |
| AC-6: Modulith 邊界合規 | VERIFIED | `ModuleArchitectureTest.modulesVerify` | PASS |
| AC-7: Provider-agnostic metadata | VERIFIED | `TurnRecorderTest.ac7_providerAgnosticMetadata` | PASS |

**Testability Gate: CLEAR** — 所有 7 個 AC 均有執行證據。
...
1. **`TurnRecorder.updateProjection()` 存在死程式碼（dead code）**
   `TurnRecorder.java` 第 107–112 行有一個空的 `.ifPresent()` block，帶有未完成的注解 `"token values extracted via side-effect is awkward; use a holder"`。這段程式碼完全沒有功能，僅是實作過程的殘留，違反開發標準禁止「無明確翻譯至領域例外的全捕獲」精神（dead code 是 drift 類型技術債）。
   ...
   extractors.stream()
           ...
           .findFirst()
           .ifPresent(e -> {
               // token values extracted via side-effect is awkward; use a holder
           });
   ```

2. **`TurnRecorder.updateProjection()` 的 `turnNumber` 參數未使用**
   方法簽章宣告 `int turnNumber` 但方法體內完全未使用此參數（Java compiler 會產生 unused parameter warning）。此參數為多餘，可移除或未來有意義（如 turn-boundary 安全檢查）時應有對應邏輯。

3. **`extractors.stream()` 在單次 `on()` 呼叫中被呼叫 4 次**（`buildMetadataJson` 1次、`resolveProvider` 1次、`updateProjection` 內 2次（其中 1次為死程式碼））。正確的呼叫次數應為 2 次，清除死程式碼後降為 3 次。雖效能影響可忽略（extractors 通常只有 1–2 個），但仍是輕微的重複邏輯。

**MINOR（風格/可讀性）：**

4. **`SessionHistoryService` 未被任何測試覆蓋** — 實作正確（委派給 ports），但 `getEvents()` 和 `findById()` 沒有測試。按 QA 策略 §2 coverage 目標（`application/service/` ≥ 75%），此服務的行覆蓋率為 0%。建議在 `TurnRecorderTest` 或獨立 test class 補測。

5. **AC-6 測試的 `@DisplayName` 不含 S017 標記** — `ModuleArchitectureTest.modulesVerify()` 的 DisplayName 為 `"AC-1 ApplicationModules.verify()..."` （這是 S002 的 AC-1，非 S017 的 AC-6）。`qa-strategy.md` 要求 `scripts/verify-spec-coverage.sh` 搜索 `@DisplayName(...)` 匹配 AC ID — 若該腳本按 `[S017]` 前綴搜尋，AC-6 將不會被找到。（現有測試本身是正確的；問題只在 ID 標記。）
...
| 檔案 | 結論 |
|------|------|
| `SessionEvent` | Javadoc 正確描述所有欄位，與實作一致 |
| `SessionProjection` | Javadoc 正確，含 `eventVersion` CAS 用途說明 |
| `EventFilter` | Javadoc 正確，工廠方法文件齊全 |
| `TurnRecorded` | Javadoc 正確 |
| `RecordingAgentSession` | Javadoc 準確，但 `fork()` 注解（Javadoc 無、`SessionConfig` 注解無）略有省略 |
| `RecordingAgentSessionRegistry` | Javadoc 準確，說明 `@Primary` 機制 |
| `TurnRecorder` | Javadoc 類別文件準確。但方法層級 `updateProjection` 的死程式碼注解具誤導性（暗示有計畫但未完成的重構）。|
| `ClaudeMetadataExtractor` | 無類別 Javadoc。按開發標準，`@Component` bean 應有說明用途的 Javadoc。|
| `ProviderMetadataExtractor` | Javadoc 準確且完整 |

---

### Design Sync Check（§2/§4 vs 實際程式碼）

下列 §7 Design Drift 已記錄的項目：已正確記錄，無補充。

**新發現的漂移（未記錄在 §7）：**

1. **`architecture.md` 尚未更新（IMPORTANT）** — §2 模組地圖仍列 `session` 模組為 "Backlog（`SessionMemoryAdvisor` 接線 + 持久化對話 session）"。§5.2 仍描述舊 `spring-ai-session-jdbc` schema（`AI_SESSION`、`AI_SESSION_EVENT`）。§2.x Backlog 仍使用舊描述。S017 的 Doc Sync 清單 `[ ] architecture.md §2`、`[ ] architecture.md §5.2`、`[ ] architecture.md §2.x Backlog` 三項均未勾選。

2. **`glossary.md` 缺少 S017 新術語（IMPORTANT）** — `Session Event`、`Session Projection`、`Turn`（S017 語意）、`ProviderMetadataExtractor`、`Compaction`（預留）五個術語尚未加入 glossary，違反 `development-standards.md §10`「每個新領域術語 → glossary 條目」的規則。

3. **`spec-roadmap.md` Doc Sync 實際狀態** — 第 1 項（估算更新）和第 6 項（S014 描述）已完成，但 spec 中仍標記為 `[ ]`（未勾選）。內容已更新但 checkbox 未打勾，造成狀態不一致。
...
| Layer | Result | Detail |
|-------|--------|--------|
| Automated tests | **PASS** | `./gradlew test` BUILD SUCCESSFUL；11 個 S017 測試全綠 |
| Coverage / Integration | **PARTIAL** | `application/service/SessionHistoryService` 零測試覆蓋；JaCoCo 報告未生成 |
| Manual verification | **N/A** | 所有 AC 可自動化 |
| Testability gate | **CLEAR** | 所有 7 個 AC 均為 VERIFIED |

**最終判決：REJECT-FIX**

所有 AC 的自動化測試通過，核心功能正確。但存在以下問題阻擋出貨：

**IMPORTANT（出貨前必修）：**
- F1. `TurnRecorder.updateProjection()` 死程式碼（空 `ifPresent` block）需移除
- F2. `TurnRecorder.updateProjection()` 未使用的 `turnNumber` 參數需移除或說明
- F3. `architecture.md` §2、§5.2、§2.x Backlog 三處需更新反映 S017 實際設計
- F4. `glossary.md` 需補 5 個新術語條目

**MINOR（建議改善）：**
- M1. `SessionHistoryService` 補測試（`application/service/` 覆蓋率 0%）
- M2. `ClaudeMetadataExtractor` 補類別 Javadoc
- M3. AC-6 測試的 `@DisplayName` 加入 `[S017]` 前綴
- M4. `spec-roadmap.md` Doc Sync 第 1、6 項 checkbox 打勾（狀態校正）

**建議：** F1+F2 為單一 PR 可快速修正的程式碼清理（< 10 分鐘）。F3+F4 為文件補齊。修正後可直接出貨，毋需再次完整 QA 週期。

---

### QA Review — Round 2 (Re-verification after fixes)
...
| Finding | Status | 驗證方式 |
|---------|--------|---------|
| F1: dead code（空 `ifPresent` block） | ✅ FIXED | `TurnRecorder.java` 第 101-137 行無死程式碼 |
| F2: unused `turnNumber` 參數 | ✅ FIXED | `updateProjection()` 簽名已移除 `turnNumber` |
| F3: `architecture.md` 未更新 | ✅ FIXED | §2 新增 session 模組、§2.x 標記已晉升、§5.2 更新 schema |
| F4: `glossary.md` 缺術語 | ✅ FIXED | 新增 Session Event / Session Projection / Turn / ProviderMetadataExtractor / Compaction + 修正 SessionMemoryAdvisor |

**建置驗證：**
```
./gradlew test --rerun-tasks  → BUILD SUCCESSFUL（全部 11 個 S017 測試通過）
./gradlew compileTestJava     → BUILD SUCCESSFUL
```

**結構修正：** 前次 QA 附加時意外刪除 `## Appendix A` 標題，已恢復。

**MINOR 項目追蹤（不阻擋出貨）：**
- M1（SessionHistoryService 無測試）：純委派 one-liner，風險低，記錄為 tech debt
- M2（ClaudeMetadataExtractor 無 Javadoc）：建議未來修正
- M3（AC-6 DisplayName 無 [S017]）：ModuleArchitectureTest 為共用測試，此行為正確
- M4（Doc Sync checkbox）：內容已正確，cosmetic issue

**最終裁定：PASS** — 所有 IMPORTANT 問題已修正，所有 7 個 AC 通過自動化驗證，程式碼品質符合開發標準。S017 可出貨。
